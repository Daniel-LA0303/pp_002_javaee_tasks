<jsp:include page="layout/header.jsp"/>
	...
	<div class="header-panel">
		<h1 class="mt-3">Project Panel</h1>
	            ...
	            <div class="form-container">
	                ...
	                >
	                    ...
	                    <div class="form-group">
	                        ...
	                        <input type="text" id="title" name="title" maxlength="100" placeholder="Enter the task title" class="form-input" required>
	                    </div>
	                    
	                    <!-- Description -->
	                    <div class="form-group">
	                        <label for="description" class="form-label">Description</label>
	                        <textarea id="description" name="description" placeholder="Enter task description" class="form-input" required></textarea>
	                    </div>
	                    ...
	                    <!-- User Assigned -->
	                    <div class="form-group">
	                        <label for="user_assigned_id" class="form-label">User Assigned</label>
	                        <select id="user_assigned_id" name="user_assigned_id" class="form-input" required>
	...
	<script>
		$(document).ready(function() {
			...
			var url = "${pageContext.request.contextPath}/get-info-by-project?projectId=" + encodeURIComponent(projectId);
			$.ajax({
			    url: url,  
			    type: 'GET',      
			    dataType: 'json', 
			 ...
			 $('#tasks-container').on('click', '.btn-complete', function(event) {
		            ...
		            $.ajax({
		            	url: '${pageContext.request.contextPath}/get-info-by-project',       
	        	        type: 'POST',           
            ...
            $('#searchInput').on('input', function() {
                ...
                // Si el campo está vacío, limpia los resultados y no realiza la solicitud
                if (keyword === "") {
                    $('#searchResults').html("");
                    return;
                }

                // Realiza la solicitud POST en AJAX
                $.ajax({
                    url: '${pageContext.request.contextPath}/user-by-id', // Reemplaza con la ruta de tu servlet
                    type: 'POST',
                    contentType: 'application/json',
                    dataType: 'json',
                    data: JSON.stringify({ 
                    ...
                    success: function(response) {
                        ...
                        $('#searchResults').html("");  // Limpia resultados anteriores

                        // Maneja la respuesta
                        if (response.message) {
                            $('#searchResults').html("<p>" + response.message + "</p>");
                        } else {
                            // Muestra los usuarios encontrados
                            ...
                            users.forEach(user => {
                                let userCardHTML = 
                                    '<div class="user-card">' +
                                        ...
                                        '<div class="user-card-body">' +
                                            ...
                                            // Solo el botón "Add to Project" se muestra con un atributo data-user-id
                                            '<div class="user-card-buttons">' +
                                ...
                                // Añade la card generada al contenedor
            ...
            $(document).on('click', '.btn-add', function() {
                const userId = $(this).data('user-id'); // Accede al valor del atributo data-user-id
                console.log('Agregar al proyecto el usuario con id:', userId);
                $('#searchInput').val('');
                $('#searchResults').empty();

                $.ajax({
                    url: '${pageContext.request.contextPath}/projects-user', // La URL del endpoint de tu servidor
            ...
            $(document).on('click', '.btn-remove', function() {
                ...
                $.ajax({
                    url: '${pageContext.request.contextPath}/projects-user', // La URL del endpoint de tu servidor
                    type: 'DELETE', // Método de la solicitud
                    contentType: 'application/json', // Tipo de contenido de la solicitud
                    data: JSON.stringify({
                    	userId: userId,
                    	projectId: parseInt(projectId)
